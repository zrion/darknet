...
typedef struct load_args {
    ...
    int show_imgs;
    int dontuse_opencv;
    int numpy_data;  // numpy input data
    float jitter;
    float resize;
    int flip;
    int gaussian_noise;
...
LIB_API float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, int numpy_data);
LIB_API void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path, int numpy_data);
